...
class PX4VLMController(Node):
    ...
    def update_flight_phase(self):
        ...
        if self.phase == "LAUNCH":
            ...
            if self.move_command == "forward":
                if abs(self.pos_y - self.target_y) < 0.1:
                    movement_complete = True
                else:
                    self.pos_y += 0.05
                    
            elif self.move_command == "right":
                if abs(self.pos_x - self.target_x) < 0.1:
                    movement_complete = True
                else:
                    self.pos_x -= 0.05  # Right is negative X in NED
                    
            elif self.move_command == "left":
                if abs(self.pos_x - self.target_x) < 0.1:
                    movement_complete = True
                else:
                    self.pos_x += 0.05  # Left is positive X in NED
            
            if movement_complete:
                self.get_logger().info(f"{self.move_command} movement complete. Entering HOVER phase...")
                self.phase = "HOVER"
                self.vlm_processing = False
    ...
    def process_vlm_command(self):
        ...
        try:
            ...
            COLOR -> ACTION MAPPING (must ALWAYS follow this mapping):
            - BLUE BLOCK = forward
            - GREEN BLOCK = right
            - PURPLE BLOCK = left

            BEHAVIOR RULES:
            1. Identify what color the block in the image is.
            2. Then given the COLOR -> ACTION mapping, correspond the color of the block to the action.
            3. Output ONLY the action word, nothing else.
            """
            ...
            # Parse command and set targets
            command = full_reply.strip().lower()
            self.get_logger().info(f"VLM Command: {command}")
            
            if "forward" in command:
                self.move_command = "forward"
                self.target_y = self.pos_y + 2.0
                self.phase = "MOVING"
                self.get_logger().info(f"Moving forward to Y={self.target_y}")
            elif "right" in command:
                self.move_command = "right"
                self.target_x = self.pos_x - 2.0  # Right is negative X
                self.phase = "MOVING"
                self.get_logger().info(f"Moving right to X={self.target_x}")
            elif "left" in command:
                self.move_command = "left"
                self.target_x = self.pos_x + 2.0  # Left is positive X
                self.phase = "MOVING"
                self.get_logger().info(f"Moving left to X={self.target_x}")
            else:
